...
| `scrollable-explainer/` | Principles + patterns for writing and building long-form scroll-driven visual explainers, with a measured teardown of the ProPublica CCS piece as the worked case study | v0.1 — reference only (no code); the long scrolling form was retired in explainers v2, but §1.5 is still the rule for script stage discipline |
| `illustrated-chapters/` | The explainers-v2 chapter form: page skeleton, disk layout, figure and palette rules, looping CSS animations that honour reduced motion, and how to check a page with no preview site | v0.1 — reference only (no code); proven on `../explainers/_process/series/form-prototype/`, read before building anything in `../explainers/` |
