...
> *Staging: run it live if the room and the tech are cooperating. Keep a mock-up prepared in advance as the fallback — contributions already collected and already run through the three questions, plus the reverse view — and switch to that without ceremony if the live run stalls. Decide on the day; build the mock-up either way.*
