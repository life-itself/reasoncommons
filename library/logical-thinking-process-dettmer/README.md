...
**Correctness over tidiness**: an earlier version of this pipeline aggressively dropped short/unpunctuated text on pages with a figure, on the theory that it was diagram-label debris duplicated by the embedded image. That turned out to also silently delete real sentences that happened to be short, or that trailed onto a figure-heavy page from the paragraph before it — genuine content loss, not just a style problem. The pipeline no longer discards any extracted text: heading-shaped fragments that aren't followed by real content get folded back into a plain paragraph instead of being deleted. The trade-off is that some genuine diagram-label soup (box labels, "205 / p. 2" style page-reference bubbles) still shows up as extra plain-text clutter on figure-heavy pages, alongside the embedded image that already shows the real diagram. Prefer that over losing text.
